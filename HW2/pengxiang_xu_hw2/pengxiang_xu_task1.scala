...
object pengxiang_xu_task1 {
  ...
  def get_grouped_data(raw_rdd: RDD[String], case_n: Int): RDD[Iterable[String]] ={
    val data_p: RDD[(String, String)] = {
      if (case_n == 1) {
        // User_id case
        raw_rdd.map(s => (s.split(",")(0), s.split(",")(1)))
      } else {
        // Business_id case
        raw_rdd.map(s => (s.split(",")(1), s.split(",")(0)))
      }
    }
...
//      frequent_candidates.foreach(println)
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Spark Configuration
    val conf = new SparkConf().setAppName("HW2 - Task 1").setMaster("local[*]")
    ...
    val groupded_data = get_grouped_data(data, case_num)
